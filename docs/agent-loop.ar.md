...
| **SEC-NEXT-01** عزل كاتب التقرير — عملية الاختبار تعمل بنفس المستخدم ويمكنها نظريًا كتابة ملف التقرير | **NOT VERIFIED** | الوجهة العشوائية (16 بايت) داخل `mkdtemp` خاص إخفاءٌ لا حدّ؛ يلزم عزل مستخدم/namespace/container واختبار عدائي *يفشل* في التلاعب |
| **SEC-NEXT-02** سقف زمني كلي — المهلة لكل ملف فقط | **NOT VERIFIED** | مُعاد إنتاجه كفجوة مفتوحة في `tests/known-gaps/sec-next-02-global-deadline.test.js` |
